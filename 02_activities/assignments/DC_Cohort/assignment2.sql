...
--QUERY 2
SELECT market_date, customer_id, transaction_time, -- just to see why they're unique rows
	dense_rank() OVER(
...
--QUERY 3
SELECT market_date, customer_id, transaction_time, visit_number
FROM(
	SELECT market_date, customer_id, transaction_time,
		dense_rank() OVER(
		PARTITION by customer_id
...
Filter the visits to dates before April 29, 2022. */
--QUERY 4
SELECT product_id, customer_id, quantity, market_date, transaction_time,
	COUNT(product_id) OVER (
		PARTITION BY customer_id, product_id
		ORDER BY market_date, transaction_time
	) AS product_count_by_customer
FROM customer_purchases
WHERE market_date < '2022-04-29'
...
SELECT market_date, total_sales, 'best day' as day_rank
FROM ranked_days
WHERE best_rank = 1

UNION

SELECT market_date, total_sales, 'worst day' as day_rank
FROM ranked_days
WHERE worst_rank = 1;
...
SELECT
    vp.vendor_name,
    vp.product_name,
    SUM(vp.derived_price * 5) AS hypothetical_revenue
...
FROM (
    ...
    JOIN (
        SELECT
            vendor_id,
            product_id,
            AVG(cost_to_customer_per_qty) AS derived_price -- avg unit price per vendor-product 
        FROM customer_purchases
        GROUP BY vendor_id, product_id
    ) pp
        ON vi.vendor_id = pp.vendor_id
       AND vi.product_id = pp.product_id
) vp
CROSS JOIN (SELECT customer_id FROM customer) c -- make each vendor-product/customer combination
GROUP BY
    vp.vendor_name,
    vp.product_name
ORDER BY
